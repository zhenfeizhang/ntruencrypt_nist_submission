...
\textsf{Salsa20} is a fast and well accepted stream cipher. It is upto 5 times
...
public key $\bfh$, we have encoded the public key
...
\ref{alg:ssdecap}. It uses a same method as \ntrukem~to convert a public key 
...
\subsubsection{Overview} We firstly give an overview of the hardness
assumptions in this proposal.
\begin{itemize}
\item For \ntrupke~and \ntrukem~schemes:
\begin{itemize}
\item The CPA security is based on the \ntru~assumption;
\item We use NAEP transformation \cite{DBLP:conf/ctrsa/Howgrave-GrahamSW05} to convert the scheme into a CCA-2 secure encryption scheme.
\end{itemize} 
...
except perhaps for NAEP transform. Therefore we
give a high level description of  NAEP and show its
connects to the well-known Fujisaki-Okamoto transform.


\subsubsection{NAEP transform}
In \cite{Fujisaki1999}, the authors proposed a generic method to 
transform the CPA secure encryption algorithm into a CCA-2 secure version.
This method is usually referred to as Fujisaki-Okamoto transform. At a high level, it works as follows. During the encryption, one firstly choose
a salt, and append it to the message. Then hash the appended message into
...
re-encrypt the message with the same salt, and compared the result ciphertext with received one. If those two does not match, abort the 
decryption.
...
CRYPTO\_SECRETKEYBYTES & \multicolumn{2}{|c|}{700} &\multicolumn{2}{|c|}{1172}&\multicolumn{2}{|c|}{8194} \\
CRYPTO\_PUBLICKEYBYTES & \multicolumn{2}{|c|}{610} &\multicolumn{2}{|c|}{1022}&\multicolumn{2}{|c|}{4097}\\
CRYPTO\_BYTES &\multicolumn{2}{|c|}{32}&\multicolumn{2}{|c|}{48} &\multicolumn{2}{|c|}{48}\\
CRYPTO\_CIPHERTEXTBYTES &\multicolumn{2}{|c|}{610}&\multicolumn{2}{|c|}{1022}&\multicolumn{2}{|c|}{4097}\\\hline
\end{tabular}
\end{table}
We address NIST's required security levels as follows:
\begin{itemize}
\item Level 1, equivalent to 128-bit block cipher: use
\ntrupke~and \ntrukem~with parameter set \ntru-443 or \ntru-743;
\item Level 2, equivalent to 256-bit hash function: use \ntrupke~and \ntrukem~with parameter set  \ntru-743;
\item Level 3, equivalent to 192-bit block cipher: use \ntrupke~and \ntrukem~with parameter set \ntru-743;
\item Level 4, equivalent to 384-bit hash function: use \ntrupke~and \ntrukem~with parameter set\ntru-743, or
(for extremely conservative purpose)
 \ssntrupke~and \ssntrukem~with parameter set \ntru-1024; 
\item Level 5, equivalent to 256-bit block cipher: use \ntrupke~and \ntrukem~with parameter set \ntru-743, or
(for extremely conservative purpose)
 \ssntrupke~and \ssntrukem~with parameter set \ntru-1024.
\end{itemize}
...
length of this lattice is $$\sqrt{qN/\pi e},$$ while the
$l_2$ norm of the target vectors are  $\|\bff,\bfg\|_2$.
 This gives
a root Hermite factor of the lattice as
$$
...
with a root Hermite factor of 1.005. %, until for the
...
the handshake payloads in a single Maximum Transmission Unit (MTU), or
one would expect some package drops that may potentially slow down 
...
\paragraph{Lack of provable security} One question that was asked 
...
We provide two arguments to address this concern. Firstly, in \cite{DBLP:conf/eurocrypt/StehleS11} it was shown that one can establish a reduction to 
...
To validate our understanding on cryptanalytic techniques, we published the \ntru~challenge. Only the first few challenges of tiny dimensions was 
